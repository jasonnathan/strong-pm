...
var minkeConfigOptions = require('./minke_config.json')
...
function Server(originalCommand, baseDir, listenPort, controlPath) {
 ...
 // set up minkelite
  this._app.use(minkeLite(minkeConfigOptions).get_express_app());
...
Server.prototype._onMasterStart = function(masterInfo, callback) {
  var self = this;
...
Server.prototype._onTraceData = function _onTraceData(req, callback) {
  debug('trace data: %j', req.trace);
  var Process = this._app.models.ServiceProcess;
  var AgentTrace = this._app.models.AgentTrace;

  var q = {where: {serviceInstanceId: 1, pid: +req.processId, stopTime: null}};
  Process.findOne(q, function(err, proc) {
    assert.ifError(err);
    if (!proc) {
  ...
  function deleteOld() {
    ...
    AgentTrace.destroyAll(where, function(err) {
      assert.ifError(err);
      if (callback) return callback(err);
    });
  }
...
Server.prototype._onRunnerRequest = function _onRunnerRequest(req, callback) {
  ...
  switch (req.cmd) {
    ...
    case 'agent:trace':
      return this._onTraceData(req, callback);
